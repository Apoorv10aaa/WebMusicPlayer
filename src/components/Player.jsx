...
export default function Player() {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      progressInterval = setInterval(updateProgress, 100); // updating every second for smoother transition
    }
      ...
      <div className="h-full bg-[#202020] bg-opacity-50 rounded-md p-2">
        ...
        <div id="player" className="h-4/5 flex items-center px-2">
          ...
          <div id="song" className="text-white flex items-center px-2 w-5/12">
            ...
            {/* <!-- Song details --> */}
            <div>
              <h1 className="text-xl font-lato hover:underline">
                {songData.songName}
              </h1>
              <p className="text-sm fotn-lato">{songData.artists}</p>
            </div>
          </div>
          <div className="flex justify-between flex-grow">
            ...
            {/* <!-- Duration --> */}
            <div>
              <span className="text-lg font-lato text-white">
                {formatTime(duration)}
              </span>
            </div>
          </div>
