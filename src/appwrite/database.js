...
export class DatabaseService {
  ...
  async addTrack({
    ...
    try {
      const trackData = await this.database.createDocument(
        conf.appwriteDatabaseId,
  ...
  async getAlbums() {
    try {
      const albums = await this.database.listDocuments(
        conf.appwriteDatabaseId,
  ...
  async getAlbumTracks(albumId) {
    try {
      const tracks = await this.database
        .getDocument(conf.appwriteDatabaseId, conf.appwriteAlbumsId, albumId)
  ...
  async recentlyPlayed(userId) {
    try {
      const recents = await this.database
        .getDocument(conf.appwriteDatabaseId, conf.appwriteUsersId, userId)
  ...
  async updateRecents(userId, trackId) {
    try {
      let recents = await this.database
        .getDocument(conf.appwriteDatabaseId, conf.appwriteUsersId, userId)
        .then((user) => user.recents);
      if (recents.length >= 7) {
